...
def test_hybrid_species_names_have_ortho_x_stack_structure():
    s = HybridSpecies(stack_names=["code", "math", "fr"])
...
def test_hybrid_coupling_tensor_shape():
    ...
    s = HybridSpecies(stack_names=stacks)
    j = s.coupling_tensor()
    assert j.shape == (4, len(stacks), 4, len(stacks))


def test_hybrid_coupling_matches_ortho_when_projection_uniform_single_stack():
    """With 1 stack and uniform projection, coupling reduces to OrthoSpecies J.

    Uniform init produces P = full((n_o, n_s), 1/n_s); for n_s=1 each row
    sums to 1, triggering the shortcut in HybridSpecies.coupling_matrix()
    that returns OrthoSpecies J directly.
    """
    s = HybridSpecies(stack_names=["code"], projection_init="uniform")
    j = s.coupling_matrix()
    assert j.shape == (4, 4)
    expected = OrthoSpecies().coupling_matrix()
    np.testing.assert_allclose(j, expected, atol=1e-6)


def test_hybrid_n_species():
    s = HybridSpecies(stack_names=["a", "b", "c", "d", "e"])
...
def test_hybrid_projection_has_well_defined_gradient():
    ...
    s = HybridSpecies(stack_names=["a", "b"], projection_init="random")
